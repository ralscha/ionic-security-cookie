...
public class UserAuthenticationErrorHandler
	...
	public void onApplicationEvent(AuthenticationFailureBadCredentialsEvent event) {
		...
		if (this.appProperties.getLoginLockAttempts() != null && principal instanceof String) {
			...
			if (result != null) {
				...
				if (failedLogins >= this.appProperties.getLoginLockAttempts()) {
				  if (this.appProperties.getLoginLockMinutes() != null) {
				    lockedOut = LocalDateTime.now().plusMinutes(this.appProperties.getLoginLockMinutes());
				  }
				  else {
				...
				this.dsl.update(APP_USER)
				    .set(APP_USER.LOCKED_OUT_UNTIL, lockedOut)
